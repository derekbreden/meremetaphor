...
async function buildSite() {
    console.log('Building site from PDF (mobile-constrained images)...');
...
`;
            ...
            // Read the template and update it
            const templatePath = path.join(__dirname, '..', 'index.html');
            let template = fs.readFileSync(templatePath, 'utf8');
            
            // Update styles with properly constrained images
            if (!template.includes('.cover-image')) {
                template = template.replace(
                    '</style>',
                    `    /* Mobile-first responsive design with constrained images */
        .cover-page {
            text-align: center;
            margin: 2em 0;
            padding: 1em;
        ...
        .cover-illustration {
            margin: 2em 0;
            /* Ensure illustration container doesn't exceed content width */
            max-width: 100%;
            overflow: hidden;
        }
        .cover-image {
        ...
        .chapter-image {
            width: 100%;
            max-width: 320px; /* Constrain to reasonable mobile size */
            height: auto;
            margin: 1em 0;
            display: block;
            margin-left: auto;
        ...
        @media (min-width: 768px) {
            ...
            .cover-image {
                max-width: 400px; /* Larger on tablet */
            }
            .chapter-image {
                max-width: 450px; /* Larger on tablet */
            }
            .subtitle {
                font-size: 1.2em;
            }
        ...
        @media (min-width: 1024px) {
            ...
            .cover-image {
                max-width: 400px; /* Keep reasonable on desktop */
            }
            .chapter-image {
                max-width: 500px; /* Original desktop size */
            }
        }
