...
async function buildSite() {
    console.log('Building site from PDF (embedded images + transparency fix + text)...');
    
    try {
        // Step 1: Extract embedded images
        await extractEmbeddedImages();
        
        // Step 2: Fix transparency issues
        await fixImageTransparency();
...
async function fixImageTransparency() {
    console.log('Fixing image transparency issues...');
    
    const imagesDir = path.join(__dirname, '..', 'images');
    const fixedDir = path.join(__dirname, '..', 'images-fixed');
    
    // Create fixed images directory
    if (fs.existsSync(fixedDir)) {
        execSync(`rm -rf "${fixedDir}"`);
    }
    ...
    try {
        ...
        imageFiles.forEach(filename => {
            ...
            const outputPath = path.join(fixedDir, filename);
            
            try {
                // Fix transparency by replacing black background with white
                execSync(`magick "${inputPath}" -fuzz 20% -fill white -opaque black "${outputPath}"`, {
                    stdio: 'pipe'
                });
                
                console.log(`    ✓ Fixed transparency: ${filename}`);
                ...
                // If fixing fails, keep the original
            }
        });
        
    } finally {
        // Clean up fixed directory
        if (fs.existsSync(fixedDir)) {
            execSync(`rm -rf "${fixedDir}"`);
        }
    }
